extension ListExtension<T> on List<T> {
  ...
  Map<T, List<T>> groupBy(T Function(T) fn) {
    return Map<T, List<T>>.fromIterable(
      map(fn).toSet(),
  ...
  List<List<T>> split(int parts) {
    ...
    return List<List<T>>.generate(
      parts,
      (int i) =>
          sublist(size * i, (i + 1) * size <= length ? (i + 1) * size : null),
    );
  }
  ...
  ///Example:
  ///```dart
  ///list.whereNotIn("key", [value", value2])
  ///```
